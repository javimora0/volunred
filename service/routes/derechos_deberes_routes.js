...
router.post('',[
    ...
    ,validar_campos], validar_jwt.validar_admin,derechos_deberes_controller.crear_derecho_deber)

router.route('/:id')
    .put([
        check('descripcion','Debe introducir una descripción válida').isLength({min:2, max:256}),
        check('id_tipo').custom(middleware.existe_tipo_derecho_deber)
        ,validar_campos], validar_jwt.validar_admin,derechos_deberes_controller.modificar_derecho_deber)

    .delete(validar_jwt.validar_admin,derechos_deberes_controller.eliminar_derecho_deber)
